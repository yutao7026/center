...
<body>
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed table-hover">
		...
		<c:forEach items="${videos}" var="video">
			<tr>
				<td style="vertical-align: middle;"><input type="checkbox" name="ids" value="${video.id}" style="margin: 0px;"></td>
				<td style="vertical-align: middle;" class="textCenter"><img src="${ctx}/file/image?fileName=${video.backgroundImageUrl}" width="200px"/></td>
				<td style="vertical-align: middle;" class="textCenter">${video.title}</td>
				<td style="vertical-align: middle;" class="textCenter">${video.secondTitle}</td>
				<%	
					if(SessionUtil.checkPurview(session)) {
						%>
						<td class="textCenter" style="vertical-align: middle;">
							<a href="${ctx}/video/update/${video.id}">编辑</a>
						</td>
						<%
					}
				%>
	...
	<%	
		if(SessionUtil.checkPurview(session)) {
			%>
			<div align="center">
				<a href="${ctx}/video/update/0" class="btn btn-primary"><span class="glyphicon glyphicon-plus-sign"></span>&nbsp;增加</a>
				&nbsp;&nbsp;&nbsp;&nbsp;
