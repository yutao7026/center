...
<body style="min-width: 1000px;">
				...
				<table class="table">
					<tr>
						...
						<td width="100px;" style="vertical-align: middle;">
							<select id="changeReport" style="width: 160px;height: 25px;padding-top: 0px;" class="form-control">
								<option value="1">页面访问总次数</option>
							</select>
						</td>
					</tr>
				</table>
	...
	<script type="text/javascript">
		...
		function getChartData(flag) {
			...
			myChart.clear();
			if(flag == 1){
				options.title.text = "页面访问总次数";
				options.yAxis[0].axisLabel.formatter = "{value} 次";
				url = "${ctx}/report/pageAccessCountAllUser";
			}
			if(flag == 2){
				options.title.text = "节目观看总时长图表";
				url = "${ctx}/report/watchTimeAllProgramData";
			}
			if(flag == 3){
				options.title.text = "节目观看总次数图表";
				url = "${ctx}/report/watchCountAllProgramData";
			}
